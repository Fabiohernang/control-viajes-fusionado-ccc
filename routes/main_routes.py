...
from models import Viaje, Factura, Pago, LiquidacionFletero, LiquidacionDescuento
...
def index():
    ...
    if cantidad_viajes_mes_anterior > 0:
        variacion_viajes = round(((cantidad_viajes_mes - cantidad_viajes_mes_anterior) / cantidad_viajes_mes_anterior) * 100)

    total_facturado_mes = to_decimal(
        db.session.query(func.coalesce(func.sum(Factura.importe_total), 0))
        .filter(func.extract("year", Factura.fecha) == hoy.year, func.extract("month", Factura.fecha) == hoy.month)
        .scalar()
    )

    total_cobrado_mes = to_decimal(
        db.session.query(func.coalesce(func.sum(Pago.total_aplicable), 0))
        .filter(func.extract("year", Pago.fecha_pago) == hoy.year, func.extract("month", Pago.fecha_pago) == hoy.month)
        .scalar()
    )
    ...
    viajes_pendientes = Viaje.query.filter_by(liquidado=False).order_by(Viaje.fecha.desc()).limit(8).all()
    ...
    return render_template("index.html", stats=stats, ultimas_facturas=ultimas_facturas, ultimos_pagos=ultimos_pagos, ultimas_liquidaciones=ultimas_liquidaciones, viajes_pendientes=viajes_pendientes, alertas=alertas)
...
def reportes():
    ...
    ).order_by(Viaje.fecha.asc(), Viaje.id.asc()).all()
    ...
    descuentos_por_concepto = db.session.query(
        LiquidacionDescuento.concepto,
        func.coalesce(func.sum(LiquidacionDescuento.importe), 0),
    ).join(LiquidacionFletero).filter(
        func.extract("year", LiquidacionFletero.fecha) == year,
        func.extract("month", LiquidacionFletero.fecha) == month,
    ).group_by(LiquidacionDescuento.concepto).order_by(LiquidacionDescuento.concepto.asc()).all()

    descuentos_liquidaciones = [
        {"concepto": concepto, "total": quantize_money(to_decimal(total))}
        for concepto, total in descuentos_por_concepto
    ]
    total_descuentos_liquidaciones = quantize_money(sum((x["total"] for x in descuentos_liquidaciones), Decimal("0")))
    ...
    return render_template(
        ...
        descuentos_liquidaciones=descuentos_liquidaciones,
        total_descuentos_liquidaciones=total_descuentos_liquidaciones,
    )


@main_bp.route("/reportes/matias")
@login_required
def reporte_matias():
    today = date.today()
    month = int(request.args.get("month", today.month))
    ...
    total_matias = quantize_money(sum((to_decimal(v.comision_lucas) for v in viajes_mes), Decimal("0")))
...
def exportar_reporte_lucas():
    ...
    writer.writerow([f"Reporte Comisión Matías {month:02d}/{year}"])
    writer.writerow([])
    writer.writerow(["Fecha", "Cliente", "Factura", "Fletero", "Origen", "Destino", "KG", "Total Importe", "Comisión Matías"])
    ...
    writer.writerow([])
    writer.writerow(["TOTAL COMISIÓN MATÍAS", "", "", "", "", "", "", "", str(total_matias)])

    csv_data = output.getvalue()
    output.close()

    filename = f"reporte_matias_{year}_{month:02d}.csv"
    return Response(csv_data, mimetype="text/csv; charset=utf-8", headers={"Content-Disposition": f"attachment; filename={filename}"})
